...
**Soumission** → `POST /api/V1/routes/signalements`
...
- `DELETE /api/V1/gdpr/account` → déconnexion immédiate
...
- **Avatar upload** avec recadrage interactif (`react-easy-crop` via `AvatarCropModal`)
  - Aperçu immédiat, `PUT /api/V1/users/profile/avatar`
  - Bouton "Retirer l'avatar" → `DELETE /api/V1/users/profile/avatar`
- Formulaire : Prénom, Nom, Email, Téléphone
- **Changement de mot de passe** : Ancien / Nouveau / Confirmation avec toggle visibilité
- `PUT /api/V1/users/profile` en soumission + `avatarVersion` bumpé pour forcer le rechargement de l'image
...
**Problème** : après `POST /api/V1/routes/signalements`, ni le citoyen, ni les gestionnaires, ni les admins ne recevaient de notification.

**Cause racine** : `app.use('/api', express.json(), gdprRoutes)` dans l'API Gateway consommait le body JSON de **toutes** les requêtes `/api/V1/*` avant que le proxy ne le transmette en amont. Le body arrivait vide aux microservices → timeout 30 s.
...
**Soumission** → `POST /api/V1/collectes/:id/anomalie`
...
**Confirmation**
- Bouton **"Terminer la tournée"** → `PUT /api/V1/tournees/:id/statut { statut: "TERMINEE" }`
...
- `GET /api/V1/notifications/unread/count` toutes les 15 secondes
...
**API Gateway — 500 `/api/V1/alerts/unified`**
...
**API Gateway — 500 `/api/V1/iot/sensors/status`**
- Cause : handler direct passait de faux headers `x-user-id: '1'` → service-iot retournait 401 → 500.
- Fix : suppression du handler direct → la route proxy `/api/V1/iot` gère désormais cet endpoint avec le vrai JWT utilisateur.
...
**API Gateway** (`/api/V1/consent`):
- `POST /api/V1/consent` - Enregistrement consentement avec preuve (IP, user-agent)
- `GET /api/V1/consent/:userId` - Historique des consentements
...
**Routes Admin** (`/api/V1/admin/notifications`):
- `GET /api/V1/admin/notifications` - Lister les notifications avec pagination
- `PATCH /api/V1/admin/notifications/{id}/read` - Marquer une notification comme lue
- `POST /api/V1/admin/notifications/read-all` - Marquer toutes les notifications comme lues
- `GET /api/V1/admin/notifications/stats` - Obtenir les statistiques des notifications

**Routes Gestionnaire**:
- `GET /api/V1/notifications/list` - Lister les notifications avec pagination
...
- `GET /api/V1/notifications/unread/count` - Obtenir le nombre de notifications non lues
...
|---------|---------|-------------|
| api-gateway | `src/index.js` | Proxy des routes `/api/V1/admin/notifications` |
...
- **Désactivation MFA** : `POST /api/V1/auth/mfa/disable`
...
**Bug corrigé**
- `POST /api/V1/routes/optimize/preview` renvoyait `500 Internal Server Error` à chaque appel.
...
- **Nouveau endpoint** `POST /api/V1/routes/optimize/preview`
...
| Endpoint | TTL | Clé |
|----------|-----|-----|
| `GET /api/V1/zones` | 30 min | `apigw:/api/V1/zones` |
| `GET /api/V1/typecontainers` | 30 min | `apigw:/api/V1/typecontainers` |
| `GET /api/V1/containers` | 5 min | `apigw:/api/V1/containers` |
| `GET /api/V1/stats` | 2 min | `apigw:/api/V1/stats` |
...
|----------|-------------|
| `/api/V1/metrics/overview` | Vue d'ensemble services + infrastructure |
| `/api/V1/metrics/services` | Santé des services avec latence/erreur |
| `/api/V1/metrics/iot` | Capteurs, conteneurs, batterie |
| `/api/V1/metrics/kafka` | Messages, consumer lag |
| `/api/V1/metrics/database` | Connexions DB, cache hit ratio |
| `/api/V1/metrics/alerts` | Alertes actives (filtrables par sévérité/service) |
| `/api/V1/metrics/alerts/counts` | Compteurs alertes par sévérité |
| `/api/V1/metrics/history` | Données historiques |
...
```javascript
// Alertes filtrées
const res = await fetch('http://localhost:3015/api/V1/metrics/alerts?severity=critical');
const { alerts, counts, total } = await res.json();

// Compteurs badge
const counts = await fetch('http://localhost:3015/api/V1/metrics/alerts/counts');
```
...
| Endpoint | Description |
|---------|-------------|
| `POST /api/V1/logs` | Créer un log |
| `GET /api/V1/logs` | Liste avec filtres |
| `GET /api/V1/logs/filters` | Valeurs disponibles |
| `GET /api/V1/logs/summary` | Statistiques globales |
| `GET /api/V1/logs/export` | Export JSON ou CSV |
