...
// Test wrapper that conditionally skips tests if DB is not available
const testIfDbAvailable = (description, testFn) => {
  if (dbAvailable) {
    it(description, testFn);
  } else {
    it.skip(`[SKIPPED - NO DB] ${description}`, testFn);
  }
};
...
describe('Container Routes - Full Integration Tests (Real Database)', () => {
  beforeAll(async () => {
    ...
    try {
      ...
      return; // Return early if DB is not available
    }

    // Initialize app and test data only if DB is available
  ...
  beforeAll(async () => {
    if (!dbAvailable) {
      console.warn('Skipping beforeAll - Database not available');
      return;
    }
  ...
  afterAll(async () => {
    if (!dbAvailable) {
      console.warn('Skipping afterAll cleanup - Database not available');
      return;
    }
