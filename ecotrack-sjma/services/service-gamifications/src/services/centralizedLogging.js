...
class CentralizedLoggingService {
  ...
  async queryLogs(options = {}) {
    ...
    const params = ['service-gamifications'];
    let paramIndex = 2;
  ...
  async getStats(days = 7) {
    const query = `
      ...
      WHERE service = 'service-gamifications'
  ...
  async cleanup(retentionDays = 30) {
    const query = `DELETE FROM centralized_logs WHERE service = 'service-gamifications' AND timestamp < NOW() - INTERVAL '${retentionDays} days'`;
    try {
      const result = await this.pool.query(query);
