...
describe('UserRepository', () => {
  ...
  describe('assignZone', () => {
    it('assigns id_gestionnaire for GESTIONNAIRE role', async () => {
      pool.query
        .mockResolvedValueOnce({ rows: [{ role_par_defaut: 'GESTIONNAIRE' }] })                          // user check
        .mockResolvedValueOnce({ rows: [{ id_zone: 1 }] })                                              // zone check
        .mockResolvedValueOnce({ rows: [{ id_zone: 1, code: 'Z01', nom: 'Centre', id_gestionnaire: 7 }] }); // UPDATE

      const result = await UserRepository.assignZone(7, 1);

      expect(result).toEqual({ id_zone: 1, code: 'Z01', nom: 'Centre', id_gestionnaire: 7 });
      expect(pool.query.mock.calls[2][0]).toContain('id_gestionnaire');
      expect(pool.query.mock.calls[2][1]).toEqual([7, 1]);
    });

    it('assigns id_admin for ADMIN role', async () => {
      pool.query
        .mockResolvedValueOnce({ rows: [{ role_par_defaut: 'ADMIN' }] })
        .mockResolvedValueOnce({ rows: [{ id_zone: 2 }] })
        .mockResolvedValueOnce({ rows: [{ id_zone: 2, code: 'Z02', nom: 'Nord', id_admin: 5 }] });

      const result = await UserRepository.assignZone(5, 2);

      expect(result.id_admin).toBe(5);
      expect(pool.query.mock.calls[2][0]).toContain('id_admin');
    });

    it('throws User not found when user does not exist', async () => {
      pool.query.mockResolvedValueOnce({ rows: [] });

      await expect(UserRepository.assignZone(99, 1)).rejects.toThrow('User not found');
    });

    it('throws 400 when role is CITOYEN', async () => {
      pool.query.mockResolvedValueOnce({ rows: [{ role_par_defaut: 'CITOYEN' }] });

      const err = await UserRepository.assignZone(1, 1).catch(e => e);
      expect(err.message).toMatch(/GESTIONNAIRE ou ADMIN/);
      expect(err.status).toBe(400);
    });

    it('throws 400 when role is AGENT', async () => {
      pool.query.mockResolvedValueOnce({ rows: [{ role_par_defaut: 'AGENT' }] });

      const err = await UserRepository.assignZone(1, 1).catch(e => e);
      expect(err.status).toBe(400);
    });

    it('throws 404 when zone does not exist', async () => {
      pool.query
        .mockResolvedValueOnce({ rows: [{ role_par_defaut: 'GESTIONNAIRE' }] })
        .mockResolvedValueOnce({ rows: [] });

      const err = await UserRepository.assignZone(7, 999).catch(e => e);
      expect(err.message).toMatch(/non trouvée/);
