...
router.get('/',
    ...
    async (req, res) => {
        try {
            const constants = await AgentPerformanceConstantsRepository.getAll();
            const formatted = {};
...
router.get('/:key',
    ...
    async (req, res) => {
        try {
            const constant = await AgentPerformanceConstantsRepository.getByKey(req.params.key);
            if (!constant) {
...
router.put('/:key',
    ...
    async (req, res) => {
        try {
            ...
            const existing = await AgentPerformanceConstantsRepository.getByKey(key);
            if (!existing) {
                return res.status(404).json({ error: 'Constante non trouvée' });
            }
            ...
            await AgentPerformanceConstantsRepository.update(key, finalValue);
