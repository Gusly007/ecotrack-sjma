...
export async function fetchMyTournee() {
  const response = await api.get('/api/V1/routes/my-tournee');
  return unwrap(response.data);
}

export async function fetchEtapes(tourneeId) {
  const response = await api.get(`/api/V1/routes/tournees/${tourneeId}/etapes`);
  return unwrap(response.data);
}

export async function fetchEtapeById(etapeId) {
  const response = await api.get(`/api/V1/routes/etapes/${etapeId}`);
  return unwrap(response.data);
}

export async function fetchProgress(tourneeId) {
  const response = await api.get(`/api/V1/routes/tournees/${tourneeId}/progress`);
  return unwrap(response.data);
}

export async function changeStatut(tourneeId, statut) {
  const response = await api.patch(`/api/V1/routes/tournees/${tourneeId}/statut`, { statut });
  return unwrap(response.data);
}

export async function recordCollecte(tourneeId, data) {
  const response = await api.post(`/api/V1/routes/tournees/${tourneeId}/collecte`, data);
  return unwrap(response.data);
}
...
export async function reportAnomalie(tourneeId, data) {
  ...
  delete payload.type;
  const response = await api.post(`/api/V1/routes/tournees/${tourneeId}/anomalie`, payload);
  return unwrap(response.data);
}

export async function fetchAnomalies(tourneeId) {
  const response = await api.get(`/api/V1/routes/tournees/${tourneeId}/anomalies`);
  return unwrap(response.data);
}

export async function fetchMapData(tourneeId) {
  const response = await api.get(`/api/V1/routes/tournees/${tourneeId}/map`);
  return unwrap(response.data);
}
...
export async function fetchKpis() {
  const response = await api.get('/api/V1/routes/stats/kpis');
  return unwrap(response.data);
}

export async function fetchAgentStats(period = 'semaine') {
  const response = await api.get('/api/V1/routes/stats/agent', { params: { period } });
  return unwrap(response.data) || {};
}
...
export async function fetchTourneeCreationOptions() {
  const [zonesResult, agentsResult, vehiclesResult, typesResult] = await Promise.allSettled([
    api.get("/api/V1/zones", { params: { page: 1, limit: 100 } }),
    api.get("/users/agents", { params: { page: 1, limit: 100 } }),
    api.get("/api/V1/routes/vehicules", { params: { page: 1, limit: 100 } }),
    api.get("/api/V1/routes/types-conteneurs"),
  ]);
...
export async function optimizeTournee(payload) {
  const response = await api.post("/api/V1/routes/optimize", payload, {
...
export async function previewOptimizeTournee(payload) {
  const response = await api.post("/api/V1/routes/optimize/preview", payload, {
    timeout: OPTIMIZE_TIMEOUT_MS,
  });
  return unwrap(response.data) || response.data;
}
...
export async function fetchTourneeById(id) {
  const response = await api.get(`/api/V1/routes/tournees/${id}`);
  return unwrap(response.data) || response.data;
}

export async function fetchTourneeEtapes(id) {
  const response = await api.get(`/api/V1/routes/tournees/${id}/etapes`);
  return extractList(response.data || {});
}

export async function fetchTourneeProgress(id) {
  const response = await api.get(`/api/V1/routes/tournees/${id}/progress`);
  return unwrap(response.data) || {};
}
...
export async function assignTourneeAgent(id, idAgent) {
  const response = await api.patch(`/api/V1/routes/tournees/${id}`, {
...
export async function updateTournee(id, data) {
  const response = await api.patch(`/api/V1/routes/tournees/${id}`, data);
  return unwrap(response.data) || response.data;
}

export async function updateTourneeStatut(id, statut) {
  const response = await api.patch(`/api/V1/routes/tournees/${id}/statut`, { statut });
  return unwrap(response.data) || response.data;
}
