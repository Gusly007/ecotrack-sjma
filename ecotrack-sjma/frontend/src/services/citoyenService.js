...
export const citoyenService = {
  // Profil
  getProfile: () => api.get('/api/V1/users/profile').then(unwrap),
  getProfileWithStats: () => api.get('/api/V1/users/profile-with-stats').then(unwrap),
  updateProfile: (data) => api.put('/api/V1/users/profile', data).then(unwrap),
  changePassword: (data) => api.post('/api/V1/users/change-password', data).then(unwrap),
  ...
  getMySignalements: (filters = {}) => {
    ...
    return api.get(`/api/V1/routes/signalements${qs ? `?${qs}` : ''}`).then((r) => {
      const body = r.data;
  ...
  getSignalementById: (id) =>
    api.get(`/api/V1/routes/signalements/${id}`).then((r) => r.data?.data?.data ?? r.data?.data ?? r.data),
  getSignalementHistory: (id) =>
    api.get(`/api/V1/routes/signalements/${id}/historique`).then((r) => r.data?.data ?? r.data),
  getSignalementTypes: () => api.get('/api/V1/routes/signalements/types').then((r) => r.data?.data ?? r.data),
  createSignalement: (data) => api.post('/api/V1/routes/signalements', data).then((r) => r.data?.data ?? r.data),

  // Collectes (prochaines tournées planifiées / en cours).
  getProchainesCollectes: ({ limit = 5 } = {}) =>
    api.get(`/api/V1/routes/prochaines-collectes?limit=${limit}`).then((r) => {
      const body = r.data;
      if (body?.data?.data) return body.data.data;
      if (body?.data) return body.data;
      return body;
  ...
  getContainers: (filters = {}) => {
    ...
    return api.get(`/api/V1/containers?${params}`).then((r) => r.data?.data ?? r.data);
  },
  getContainerById: (id) => api.get(`/api/V1/containers/id/${id}`).then((r) => r.data?.data ?? r.data),
  getContainerByUid: (uid) => api.get(`/api/V1/containers/uid/${uid}`).then((r) => r.data?.data ?? r.data),

  // Gamification
  getMyStats: (userId) => api.get(`/api/V1/gamification/stats/utilisateurs/${userId}/stats`).then((r) => r.data),
  ...
  getDefis: ({ userId } = {}) => {
    ...
    return api.get(`/api/V1/gamification/defis${qs}`).then((r) => r.data);
  },

  getBadges: () => api.get('/api/V1/gamification/badges').then((r) => r.data),
  getMyBadges: (userId) => api.get(`/api/V1/gamification/badges/utilisateurs/${userId}`).then((r) => r.data),
  ...
  getClassement: ({ limite = 10, idUtilisateur } = {}) => {
    ...
    const qs = params.toString();
    return api.get(`/api/V1/gamification/classement${qs ? `?${qs}` : ''}`).then((r) => r.data);
  },
  ...
  getMyHistorique: (userId, { limit = 100 } = {}) =>
    api
      .get(`/api/V1/gamification/stats/utilisateurs/${userId}/historique?limit=${limit}`)
      .then((r) => r.data?.data ?? r.data),

  // RGPD — Article 15 (export) et Article 17 (suppression).
  exportMyData: () => api.get('/api/V1/users/me/export').then((r) => r.data?.data ?? r.data),
  deleteAccountRequest: (password) =>
    api.delete('/api/V1/users/me', { data: { password } }).then((r) => r.data),
