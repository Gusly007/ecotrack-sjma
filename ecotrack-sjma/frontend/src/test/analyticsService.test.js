...
describe('analyticsService', () => {
  ...
  it('predict methods call expected endpoints', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/api/V1/analytics/ml/predict', {
      containerId: 10,
      daysAhead: 3,
      includeWeather: true,
    });
    ...
    expect(api.get).toHaveBeenCalledWith('/api/V1/analytics/ml/predict-critical', {
  ...
  it('aliases delegate to prediction/anomaly methods', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith('/api/V1/analytics/ml/anomalies/22', {
      params: { threshold: 2.5 },
    });
    expect(api.get).toHaveBeenCalledWith('/api/V1/analytics/ml/defective-sensors');
  });

  it('anomaly endpoints call expected URLs', async () => {
    await analyticsService.detectAnomalies(7, 3);
    expect(api.get).toHaveBeenCalledWith('/api/V1/analytics/ml/anomalies/7', {
      params: { threshold: 3 },
    });

    await analyticsService.detectAnomaliesAndCreateAlerts(8, 2, false);
    expect(api.post).toHaveBeenCalledWith('/api/V1/analytics/ml/anomalies/8/alerts', null, {
      params: { threshold: 2, autoCreate: false },
    });

    await analyticsService.detectGlobalAnomalies(1.5, 10);
    expect(api.get).toHaveBeenCalledWith('/api/V1/analytics/ml/anomalies/global', {
  ...
  it('dashboard and KPI methods call expected endpoints', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith('/api/V1/analytics/critical-containers', {
      params: { threshold: 80, limit: 12 },
    });
    expect(api.get).toHaveBeenCalledWith('/api/V1/containers', {
      params: { limit: 50, page: 1 },
    });
    expect(api.get).toHaveBeenCalledWith('/api/V1/analytics/kpis', {
      params: { period: '30d' },
    });
  });
  ...
  it('metrics/trends/weather/cache methods return response data', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith('/api/V1/analytics/performance/environmental?t=12345');

    await analyticsService.getFillTrends(14);
    expect(api.get).toHaveBeenCalledWith('/api/V1/analytics/aggregation/fill-trends', {
      params: { days: 14 },
    });

    await analyticsService.getZonePerformance();
    expect(api.get).toHaveBeenCalledWith('/api/V1/analytics/aggregation/zone-performance');

    await analyticsService.getWeatherImpact();
    expect(api.get).toHaveBeenCalledWith('/api/V1/analytics/weather-impact');

    await analyticsService.getCollecteStats(60);
    expect(api.get).toHaveBeenCalledWith('/api/V1/analytics/collecte-stats', {
      params: { days: 60 },
    });

    await analyticsService.getMLPredictions(2, 70);
    expect(api.get).toHaveBeenCalledWith('/api/V1/analytics/ml/predictions', {
      params: { daysAhead: 2, threshold: 70 },
    });

    await analyticsService.refreshCache();
    expect(api.post).toHaveBeenCalledWith('/api/V1/analytics/cache/refresh');
